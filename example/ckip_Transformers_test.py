...
for start in range(0, len(titles), chunk_size):
    ...
    for r in chunk_results:
        ...
        results.append({
            "label": sentiment,
            "score": round(r["score"], 4), #模型對這個判斷的信心強度, 0 ~ 1（永遠正數）
...
for i in range(min(20, len(ws_result))):
    print(f"\n原始標題：{titles[i]}")
    #print(f"斷詞結果：{words}")
    print(f"斷詞結果：{' / '.join(ws_result[i])}")
    print(f"情緒標籤：{results[i]['label']}")
    print(f"情緒分數：{results[i]['score_normalized']:+.4f}")
    print("-" * 40)

# --- 儲存結果 ---
df.to_csv("score_data/news_sentiment_wang.csv", index=False, encoding="utf-8-sig")
